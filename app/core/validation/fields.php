...
abstract class FieldsValidator {
  private $request;
	...
	public static function Validation(Array $request, array $rules = NULL, $id = NULL) {
		if(!$rules) {
			return false;
		}
				...
				if (!empty($rField)) {
					foreach ($field_rules as $value) {
						...
						if ($rule_name !== 'required') {
							if ($rule_name === 'unique') {
								$index_field = 'id';
								$result      = self::$rule_method($rule_data[1], $rule_data[2], $rField, $id, $index_field);
								if (!empty($result)) {
									$response[$field][$rule_name] = $result;
								}
							} elseif ($rule_name === 'exist') {
								$result = self::$rule_method($rField, $rule_data[1], $rule_data[2]);
								if (!empty($result)) {
									$response[$field][$rule_name] = $result;
								}
							} elseif (isset($rule_data[1]) && !empty($rule_data[1])) {
								$result = self::$rule_method($rule_data[1], $rField);
								if (!empty($result)) {
									$response[$field][$rule_name] = $result;
								}
							} else {
								$result = self::$rule_method($rField);
								if (!empty($result)) {
									$response[$field][$rule_name] = $result;
								}
							}
  ...
  private static function max($value, $var) {
    if (strlen($var) > $value) {
  ...
  private static function min($value, $var) {
    if (strlen($var) < $value) {
  ...
  private static function alpha_num($var) {
  ...
  private static function alpha_dash($var) {
  ...
  private static function alpha($var) {
  ...
  private static function base64($var) {
    $regex = "/([^a-zA-Z0-9\/\+\=]+)/";
    if (preg_match($regex, $var)) {
  ...
  private static function alpha_spaces($var) {
  ...
  private static function numeric($var) {
    if (!is_numeric($var)) {
  ...
  private static function max_value($value, $var) {
    if ($var > $value) {
  ...
  private static function min_value($value, $var) {
    if ($var < $value) {
   ...
   */
  private static function unique(string $table, string $field, string $value, $id, string $index_field) {
    if ($id || $id != 0) {
  ...
  private static function date_format($date) {
    $date = explode('-', $date);
  ...
  private static function time_format($value) {
  ...
  private static function exist($id, string $table, string $row) {
    $get_data = [
      'table'   => $table,
      'filters' => [
        [$row, $id, '='],
  ...
  private static function decimal(string $number) {
    $response = false;
    $regex    = "[^0-9\.]";
  ...
  private static function rfc(string $rfc) {
